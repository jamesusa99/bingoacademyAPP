  ...
  function NavBreadcrumb({ label }) {
    return (
      <div className="flex items-center gap-2 mb-6">
        <button onClick={() => setSection('home')} className="text-sm text-slate-500 hover:text-primary transition">科学研学首页</button>
      ...
      <section className="mb-10">
        <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-bingo-dark via-slate-800 to-cyan-900 text-white p-8 sm:p-12">
          <div className="relative z-10 max-w-2xl">
            <p className="text-xs text-cyan-300 mb-2 tracking-wider">Bingo Academy · 科学研学专场</p>
            <h1 className="text-3xl sm:text-4xl font-bold leading-tight mb-3">科创筑梦，从这里出发</h1>
            <p className="text-slate-300 text-sm mb-2">分龄适配 · 高校联动 · 竞赛衔接 · 升学赋能</p>
            <p className="text-slate-400 text-xs mb-6">覆盖6-18岁全龄段，AI通识 · 数据科学 · 机器人 · 航天 · 科研实训，一站式科创成长平台</p>
            <div className="flex flex-wrap gap-3">
              <button onClick={() => setSection('projects')} className="bg-primary hover:bg-cyan-500 text-white px-6 py-2.5 rounded-xl text-sm font-medium transition">浏览精品项目</button>
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
        {PROJECTS.filter(p =>
          ...
          <div key={p.id} className="card p-6 hover:shadow-md hover:border-primary/30 transition group cursor-pointer"
            onClick={() => { setSelectedProject(p); setSection('project-detail') }}>
